...
3. **通知配置 (Notification Setup)**：承担下方通讯管道的配置功能。注意：外层需增加 `p-6` 内边距，且主题色块采用偏红的主色调，避免泛用蓝紫色 (`indigo`) 的生硬感。整体分为三大风琴面板（SMTP邮件服务器、Webhook频道设置、系统级通知事件分发）。
4. **投递队列与日志 (Delivery Logs)**：展示系统所有外发队列状态。在表头（<th>）直接内嵌筛选器（参考详细预约记录表格）进行数据搜索 (单号) 与状态维度 (失败、重发等) 的过滤。
...
* 扩展支持 12 个主要事件触发体系（含违规申诉、白名单、预约审批等）。
* **交互优化**：所有事件平铺为一个整洁列表，每个列表行附带【快速启用】开关以供一次性开关事件。点击某一行时，自右侧展示 **Drawer (抽屉面板)**。
* **抽屉内部**：展示该事件的独立邮件与 Webhook 配置区，可独立设定接收目标 `[✓] 用户本人`，`[ ] 系统管理员` 及各种模板编辑。
* Drawer 中的特定事件面板同时配有基于此上下文的【试发测试按钮】。
