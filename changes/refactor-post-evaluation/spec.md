...
### 2.1 核心分层治理 (P0)
- **`violation` 模块彻底解耦**：当前 `violation/routes.ts` 达 1011 行，包含超过 50 处 `db.prepare` 的直接调用和大量业务计算逻辑。目标是将其所有直接的数据库查询、聚合逻辑、违规判定算法全部下沉到 `violation/service.ts`。`routes.ts` 只应负责参数校验、服务调用和 HTTP 响应格式化。
- **设置服务职责归位**：提取并移动 `getSettingsMap` 函数。该函数目前位于 `notification/service.ts`，但被多个非通知模块调用。应该迁移回 `settings/service.ts`，以保持单一职责原则，消除通知模块承担的额外职责。

### 2.2 接口一致性与类型安全 (P2 & P3)
...
### 2.3 测试基线加固 (P1)
- 当前 135 个测试全部为通过 `supertest` 发起的 HTTP 集成测试，虽然流程覆盖率高，但对深层业务逻辑粒度较粗。
- 需要为高复杂度的纯业务核心（如预约冲突校验 `ReservationService.create`，违规计分与处罚生成 `evaluatePenaltiesOnViolation`、`checkUserPenalty`）补充直接调用 Service 层的纯逻辑单元测试。
...
- **路由瘦身原则**：路由文件 (`routes.ts`) 中的单行代码原则上不超过 150-200 行，禁止出现任何直接操作数据库（如 `db.prepare`）或处理领域核心算法的代码。
