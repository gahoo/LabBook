...
import { Clock, DollarSign, Zap, QrCode, X } from 'lucide-react';
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {equipment.map(eq => (
          <div key={eq.id} className="bg-white rounded-2xl border border-neutral-200 p-6 shadow-sm hover:shadow-md transition-shadow flex flex-col relative group">
            <button 
              onClick={() => setQrModal(eq)}
              className="absolute top-4 right-4 p-2 text-neutral-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-full transition-colors opacity-0 group-hover:opacity-100"
              title="显示二维码"
            >
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-neutral-600 mb-6 line-clamp-3">{eq.description}</p>
