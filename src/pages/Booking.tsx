...
export default function Booking() {
  ...
  const handleTimeGridClick = (date: Date, time: string) => {
    ...
    if (isNewSelection) {
      ...
      if (
        clickedTime <= start ||
        clickedTime.getTime() - start.getTime() > 24 * 60 * 60 * 1000
      ) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 403 && data.structured_penalty) {
          setBannedErrorMsg(
            `${formData.student_name}（学号：${formData.student_id}）${data.error}`,
          );
          const penalty = {
            ...data.structured_penalty,
  ...
  if (bookingStatus) {
    ...
    const showCodeOnWeb =
      bookingCodeDelivery?.web === "true" ||
      bookingCodeDelivery?.web === true ||
      bookingCodeDelivery?.web === undefined;
    ...
    return (
      <div className="max-w-md mx-auto mt-12 bg-white p-8 rounded-2xl shadow-sm border border-neutral-200 text-center">
        <div
          className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-6 ${isApproved ? "bg-emerald-100" : "bg-amber-100"}`}
        >
          <CheckCircle2
            className={`w-8 h-8 ${isApproved ? "text-emerald-600" : "text-amber-600"}`}
          />
        </div>
        <h2 className="text-2xl font-bold text-neutral-900 mb-2">
          {isApproved ? "预约成功！" : "预约已提交！"}
        </h2>
        <div className="mb-8 p-4 rounded-xl bg-neutral-50 border border-neutral-100">
          <p className="text-neutral-500 mb-2">您的预约状态为</p>
          <p
            className={`text-4xl font-black ${isApproved ? "text-emerald-600" : "text-amber-500"}`}
          >
            {isApproved ? "已通过" : "待审批"}
          </p>
        </div>
          ...
          <div className="bg-orange-50 rounded-xl p-5 mb-8 border border-orange-200 text-left">
            ...
            <p className="text-sm text-orange-700 leading-relaxed">
              ...
              <button
                ...
                className="underline font-bold text-orange-800 hover:text-orange-900 mx-1"
              >
                违规记录
              </button>
              ，此笔预约及后续预订将受到惩罚性计费处理。 当前计费倍率为{" "}
              <strong className="text-lg text-orange-900 mx-1">
                {successStructuredPenalty.restrictions.fee_multiplier} 倍
              </strong>
              。
            </p>
          </div>
        ...
        <div className="bg-neutral-50 rounded-xl p-6 mb-8 border border-neutral-200">
          <p className="text-sm text-neutral-500 mb-2 uppercase tracking-wider font-semibold">
            您的预约码
          </p>
          {showCodeOnWeb ? (
            ...
            <p className="text-lg font-medium text-neutral-800">
              预约码已通过 {deliverEmail && "Email"}{" "}
              {deliverEmail && deliverWebhook && " 及 "}{" "}
              {deliverWebhook && webhookAlias} 发送
            </p>
          )}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-red-900">
                {structuredPenalty?.restrictions?.reduce_days > 0 &&
                structuredPenalty?.penalty_method !== "BAN"
                  ? "账号受限，提前预约天数被限制"
                  : "账号受限，预约失败"}
              </h3>
              ...
              <div className="grid grid-cols-2 gap-3 mb-6">
                <div className="bg-white p-4 rounded-xl border border-neutral-200">
                  ...
                  <div className="font-medium">
                    {structuredPenalty.student_name} (
                    {structuredPenalty.student_id})
                  </div>
                </div>
                <div className="bg-white p-4 rounded-xl border border-neutral-200">
                  <div className="text-xs text-neutral-500 mb-1">解封时间</div>
                  <div className="font-medium text-red-600">
                    {structuredPenalty.unban_time
                      ? format(
                          new Date(structuredPenalty.unban_time),
                          "yyyy-MM-dd HH:mm",
                        )
                      : "未知 / 动态评估"}
                  </div>
                ...
                {filteredRules.map((rule, idx) => (
                  ...
                  >
                    ...
                    <span className="text-xs font-bold px-2 py-1 bg-red-50 text-red-700 rounded-md">
                      {rule.penalty_method === "BAN"
                        ? rule.duration_days
                          ? `封禁 ${rule.duration_days} 天`
                          : "封禁"
                        : rule.penalty_method === "REQUIRE_APPROVAL"
                          ? "转为需审批"
                          : "使用受限"}
                    </span>
                  </div>
                  ...
                  >
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        ...
                        <div className="text-sm font-medium text-neutral-900">
                          {format(
                            new Date(v.violation_time),
                            "yyyy-MM-dd HH:mm",
                          )}
                        </div>
                        <div className="text-xs text-neutral-500 mt-1">
                          关联仪器：{v.equipment_name}
                        </div>
                        {v.booking_code && (
                      ...
                      <div className="flex flex-col items-end gap-2">
                        <div className="flex flex-col items-end gap-1.5">
                          ...
                          {isRejected && (
                            <span className="text-[10px] font-medium text-red-700 bg-red-50 px-1.5 py-0.5 rounded-md border border-red-100">
                              申诉已驳回
                            </span>
                          )}
                          {isAppealing && (
                            <span className="text-[10px] font-medium text-amber-600 bg-amber-50 px-1.5 py-0.5 rounded-md border border-amber-100">
                              申诉处理中
                            </span>
                          )}
                        </div>
                        {!isApproved && !isRejected && !isAppealing && (
                          <button
                            onClick={() =>
                              setAppealingId(appealingId === v.id ? null : v.id)
                            }
                    ...
                    {remarkObj.appeal_reason && (
                      <div className="text-xs text-blue-700 bg-blue-50/50 p-2 rounded mt-2">
                        <span className="font-medium">您的申诉：</span>
                        {remarkObj.appeal_reason}
                      </div>
                    )}
                    ...
                    {remarkObj.appeal_reply && (
                      <div className="text-xs text-purple-700 bg-purple-50/50 p-2 rounded mt-2">
                        <span className="font-medium">处理回复：</span>
                        {remarkObj.appeal_reply}
                      </div>
                    )}
            ...
            <div className="mt-2 space-y-1">
              ...
              <div className="flex flex-col gap-2 text-sm text-neutral-500">
                <span className="flex items-center gap-1">
                  ...
                  {equipment.location || "未知地点"}
                </span>
                <span className="flex items-center gap-1">
                  <DollarSign className="w-4 h-4" />¥{equipment.price}/
                  {equipment.price_type === "hour" ? "小时" : "次"}
                  {equipment.consumable_fee > 0 &&
                    ` + ¥${equipment.consumable_fee}/个 耗材费`}
                </span>
                <div
                ...
                >
                  ...
                  <div
                    className={`whitespace-pre-wrap ${!showFullDesc ? "hidden md:line-clamp-2" : ""}`}
                  >
                    {equipment.description}
                  </div>
                  {!showFullDesc && (
                    <span className="text-red-600 text-xs mt-0.5 md:hidden">
                      点击查看详情
                    </span>
                  )}
                </div>
          ...
          {needsWhitelist ? (
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-neutral-200">
              ...
              <p className="text-neutral-600 mb-8">
                此仪器已开启白名单限制，您需要提交申请并获得管理员批准后方可预约使用。
              </p>

              <form
                onSubmit={handleApplyWhitelist}
              ...
              >
                ...
                <div className="flex gap-4">
                  <button
                    type="button"
                    onClick={() => setNeedsWhitelist(false)}
                              ...
                              <button
                                ...
                                className={clsx(
                                  ...
                                  isSelected
                                    ...
                                    : "bg-white hover:bg-neutral-50 group-hover:bg-neutral-50 shadow-[1px_0_4px_rgba(0,0,0,0.05)]",
                                )}
                              >
                                ...
                                {row.times.map((t, i) => {
                                  ...
                                  if (startTime && endTime) {
                                    ...
                                    if (
                                      ...
                                      if (
                                        blockEnd.getTime() === selEnd.getTime()
                                      ) {
                                        isLastSelected = true;
                                      }
                                    }
